...
export function processRailwayPNR(pnrData) {
  // Your code here
  if (!pnrData || typeof pnrData !== 'object' || !pnrData.pnr) return null;
  
  const { pnr, train, classBooked, passengers: pList } = pnrData;
  
  if (typeof pnr !== 'string' || pnr.length !== 10 || !/^\d{10}$/.test(pnr)) return null;
  if (!train || typeof train !== 'object') return null;
  if (!Array.isArray(pList) || pList.length === 0) return null;
  ...
  const processedPassengers = pList.map(p => {
    let statusLabel;
  ...
  const getCount = (label) => processedPassengers.filter(p => p.statusLabel === label).length;

  const summary = {
    totalPassengers: processedPassengers.length,
